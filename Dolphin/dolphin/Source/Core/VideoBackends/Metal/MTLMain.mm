...
void Metal::VideoBackend::PrepareWindow(WindowSystemInfo& wsi)
{
  ...
  if (wsi.type != WindowSystemType::MacOS)
    return;
  // If the caller already provided a CAMetalLayer (e.g. OpenEmu), skip the
  // NSView → CAMetalLayer setup — calling setWantsLayer: on a CAMetalLayer crashes.
  if (wsi.render_surface &&
      [(__bridge id)wsi.render_surface isKindOfClass:[CAMetalLayer class]])
    return;
