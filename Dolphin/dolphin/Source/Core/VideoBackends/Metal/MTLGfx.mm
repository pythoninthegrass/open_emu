...
// OpenEmu integration: external render target texture.
// When non-nil, BindBackbuffer renders directly into this texture and
// PresentBackbuffer skips the drawable present (OE composites the texture itself).
static id<MTLTexture> s_oe_texture = nil;

// Free function — called from DolHost.mm without pulling in MRCHelpers.h.
namespace Metal {
void GfxSetOETexture(id<MTLTexture> texture) { s_oe_texture = texture; }
}
...
bool Metal::Gfx::BindBackbuffer(const ClearColor& clear_color)
{
  @autoreleasepool
  {
    ...
    if (s_oe_texture) {
      // OpenEmu path: render directly into the provided texture — no drawable needed.
      m_backbuffer->UpdateBackbufferTexture(s_oe_texture);
    } else {
    ...
    return s_oe_texture != nil || m_drawable != nullptr;
  }
}
...
void Metal::Gfx::PresentBackbuffer()
{
  @autoreleasepool
  {
    ...
    if (s_oe_texture) {
      // OpenEmu path: OE composites the texture itself — nothing to present.
